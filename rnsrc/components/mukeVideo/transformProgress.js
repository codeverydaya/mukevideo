...
export default class Progress extends Component {
    ...
    onStart(e, g) {
        //获取 按钮的 x的位置
        let {isPortrait} = this.props;
        if(isPortrait){
            this.pageX = e.nativeEvent.pageX;
        }else{
            this.pageX = e.nativeEvent.pageY;
        }
        this.isMove = true;
    }
    ...
    onMove(e, g) {
        ...
        if (pageX< this.progressLocation.pageX) {
            this.pageX = this.progressLocation.pageX;
        } else if (pageX> (this.progressLocation.pageX + this.progressLocation.width - 10)) {
            ...
            <View onLayout={(event) => {
                {
                   ...
                   // let {x, y, width, height} = event.nativeEvent.layout;
                    ...
                    NativeModules.UIManager.measure(event.target, (x, y, width, height, pageX, pageY) => {
                        ...
                        //alert((pageX - Util.getWidth())+"----"+pageX+'----'+x+'---'+width+"---"+Util.getWidth())
